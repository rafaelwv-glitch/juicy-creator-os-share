...
import { BROWSER_HEADER, loungeUserIdFromBrowser, sanitizeBrowserStoreId } from "./browser-store";
...
export async function resolveLoungeUserId(
  request: Request,
  body?: { deviceToken?: string; browserStoreId?: string } | null,
): Promise<string | null> {
  ...
  const browserId = sanitizeBrowserStoreId(
    request.headers.get(BROWSER_HEADER) || body?.browserStoreId,
  );
  ...
  return sessionId || deviceId || browserUser || (!authConfigured ? DEV_USER_ID : null);
}
...
export async function withResolvedUserStore<T>(
  ...
  const b =
    body && typeof body === "object"
      ? (body as { deviceToken?: string; browserStoreId?: string })
      : null;
