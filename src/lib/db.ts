...
/**
 * Active backend: real **Postgres** (Neon in production, Docker locally) when
 * `DATABASE_URL` is set, otherwise embedded **PGLite**. The `"neon"` label is
 * historical — any `postgres://` URL uses the same node-postgres pool.
 */
...
export const pglitePersistent = dbSource === "pglite" && Boolean(pgliteDataDir);

/** True when DATABASE_URL points at loopback (docker-compose / local Postgres). */
export function isLocalDatabaseUrl(url = databaseUrl): boolean {
  if (!url) return false;
  try {
...
export const localPostgres = dbSource === "neon" && isLocalDatabaseUrl();
...
async function loadPgliteFsBundle(): Promise<Blob | undefined> {
  // Nitro traces the JS module into `/var/task/_libs/` on Vercel but not the
  // WASM data file. `scripts/bundle-pglite-assets.mjs` copies it there at build.
...
async function createPgliteSql(): Promise<Sql> {
  ...
  globalRef.__pgliteInstance__ ??= (async () => {
    ...
    if (dataDir) {
      console.warn("[db] PGLite file-backed at", dataDir);
    }
    return pg;
  })().catch((err) => {
 ...
 *
 * - **PGLite** (preview / no `DATABASE_URL`): open the DB (file-backed locally,
 *   in-memory on serverless) and apply `migrations/*.sql`. Idempotent.
 * - **Postgres / Neon**: no-op (pool is created lazily on first query).
 *
...
if (typeof window === "undefined" && dbSource === "pglite") {
  globalBoot.__pgBootstrapPromise__ ??= ensureDbReady().catch((err) => {
    ...
    // Don't rethrow — an unhandled rejection kills the Vercel isolate (exit 128)
    // and turns every /api/auth call into an empty 500. The next getSql() retries.
  });
}
