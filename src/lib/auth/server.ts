...
/** Stable across Vercel isolates when BETTER_AUTH_SECRET is not injected. */
function deployAuthSecret(): string {
  const material = [
    env("VERCEL_PROJECT_PRODUCTION_URL") ?? "",
    env("VERCEL_GIT_REPO_ID") ?? "",
    PREVIEW_CLIENT_SECRET,
    "juicy-lounge-auth-v1",
  ].join("|");
  return createHash("sha256").update(material).digest("hex");
}

function vercelPublicOrigin(): string | undefined {
  const strip = (v: string | undefined) => v?.replace(/^https?:\/\//, "").replace(/\/+$/, "");
  const prod = strip(env("VERCEL_PROJECT_PRODUCTION_URL"));
  const current = strip(env("VERCEL_URL"));
  const host = env("VERCEL_ENV") === "production" ? prod || current : current || prod;
  return host ? `https://${host}` : undefined;
}

// Shareable clone: auth is OFF unless VITE_AUTH_ENABLED is explicitly "true".
...
if (typeof process !== "undefined" && process.env.VERCEL) {
  console.warn(
    ...
    "grokClient",
    grokClientId ? grokClientId.slice(0, 16) : "NONE",
  );
}
...
const previewAllowedHosts: string[] = [...PREVIEW_ALLOWED_HOSTS, "*.vercel.app"];
...
const baseURL = explicitBaseURL ?? {
  ...
  // (not only the preview wildcard). Vercel hosts so GitHub-linked deploys that
  // lack BETTER_AUTH_URL still mint a valid OAuth redirect_uri.
...
export const auth = betterAuth({
  ...
  secret: env("BETTER_AUTH_SECRET") ?? (env("VERCEL") ? deployAuthSecret() : previewAuthSecret()),
  database,
  ...
  account: {
    ...
    // Cookie OAuth state so Google/X callbacks survive Vercel isolates when
    // there is no shared Postgres (verification rows would otherwise vanish).
  ...
  // Cache the session in the signed `session_data` cookie so reads skip the DB.
  // 7 days matches Better Auth's default session lifetime — required on Vercel
  // when PGLite is per-isolate and DATABASE_URL is not set.
  ...
  databaseHooks: {
    user: {
      create: {
        after: async () => {
          const { persistAuthToDurable } = await import("./auth-durable");
          await persistAuthToDurable();
        },
      },
      update: {
        after: async () => {
    ...
    ? {
        emailAndPassword: {
          ...
          sendResetPassword: async ({
            ...
            const { appResetUrl, savePendingReset, maybeSendResetEmail } = await import(
              "./password-reset-store"
            );
            const url = appResetUrl(token);
            await savePendingReset({
              email: user.email,
              userId: user.id,
              token,
              url,
            ...
            const mailed = await maybeSendResetEmail(user.email, url);
            console.warn("[auth] password reset issued", user.email, mailed ? "emailed" : "stored");
          },
        },
      }
    : {}),
